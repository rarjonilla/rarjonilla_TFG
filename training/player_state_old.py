...
class Player_state:
    ...
    def heuristics(self, played_card: Card, playable_hand: List[Card]):
        ...
        is_initial_suit_trump: bool = True if not is_first_turn and self.__trump.is_same_suit(initial_card.get_suit_id()) else False
        ...
        for played_c in self.__round_played_cards:
            if self.__trump.is_same_suit(played_c.get_suit_id()):
                trump_used = True
                if played_c.has_higher_value(highest_trump_value) or (played_c.has_same_value(highest_trump_value) and played_c.is_higher_label(highest_trump_label)):
                    highest_trump_label = played_c.get_label()
                    highest_trump_value = played_c.get_value()
                    highest_trump = played_c

            if played_c.is_same_suit(initial_card.get_suit_id()) and played_c.has_higher_value(highest_initial_suit_value) or (played_c.has_same_value(highest_initial_suit_value) and played_c.is_higher_label(highest_initial_suit_label)):
        ...
        if not is_first_turn and not trump_used and played_card.is_same_suit(initial_card.get_suit_id()) and (played_card.has_higher_value(highest_initial_suit_value) or (played_card.has_same_value(highest_initial_suit_value) and played_card.is_higher_label(highest_initial_suit_label))):
            wins_round = True
        elif not is_first_turn and not trump_used and played_card.is_same_suit(self.__trump.get_suit_id()) and (played_card.has_higher_value(highest_trump_value) or (played_card.has_same_value(highest_trump_value) and played_card.is_higher_label(highest_trump_label))):
            wins_round = True
        elif not is_first_turn and trump_used and played_card.is_same_suit(self.__trump.get_suit_id()) and (played_card.has_higher_value(highest_trump_value) or (played_card.has_same_value(highest_trump_value) and played_card.is_higher_label(highest_trump_label))):
            wins_round = True
        ...
        if teammate_played_position != -1:
            ...
            if not trump_used and teammate_played_card.is_same_suit(initial_card.get_suit_id()) and teammate_played_card.has_same_value(highest_initial_suit_value) and teammate_played_card.get_label() == highest_initial_suit_label:
                teammate_wins_round = True
            elif not trump_used and teammate_played_card.is_same_suit(self.__trump.get_suit_id()) and  teammate_played_card.has_same_value(highest_trump_value) and teammate_played_card.get_label() == highest_trump_label:
                teammate_wins_round = True
            elif trump_used and teammate_played_card.is_same_suit(self.__trump.get_suit_id()) and teammate_played_card.has_same_value(highest_trump_value) and teammate_played_card.get_label() == highest_trump_label:
                teammate_wins_round = True
        ...
        for card_in_hand in playable_hand:
            if played_card.is_same_suit(self.__trump.get_suit_id()) and card_in_hand.is_same_suit(self.__trump.get_suit_id()) and played_card.has_higher_value(card_in_hand.get_value()):
                if not is_first_turn and not trump_used and card_in_hand.is_same_suit(initial_card.get_suit_id()) and (
                    ...
                    if lower_card_trump_suit_to_win_round is None or lower_card_trump_suit_to_win_round.has_higher_value(card_in_hand.get_value()):
                        lower_card_trump_suit_to_win_round = card_in_hand
                elif not is_first_turn and not trump_used and card_in_hand.is_same_suit(
                        self.__trump.get_suit_id()) and (played_card.has_higher_value(highest_trump_value) or (
                    ...
                    if lower_card_trump_suit_to_win_round is None or lower_card_trump_suit_to_win_round.has_higher_value(card_in_hand.get_value()):
                        lower_card_trump_suit_to_win_round = card_in_hand
                elif not is_first_turn and trump_used and card_in_hand.is_same_suit(self.__trump.get_suit_id()) and (
            ...
            if not is_first_turn:
                if not trump_used and card_in_hand.is_same_suit(initial_card.get_suit_id()) and (card_in_hand.is_as() or card_in_hand.is_three()) and (card_in_hand.has_higher_value(highest_initial_suit_value) or (card_in_hand.has_same_value(highest_initial_suit_value) and card_in_hand.is_higher_label(highest_initial_suit_label))):
                    can_win_with_high = True

                if not trump_used and card_in_hand.is_same_suit(initial_card.get_suit_id()) and (card_in_hand.has_higher_value(highest_initial_suit_value) or (card_in_hand.has_same_value(highest_initial_suit_value) and card_in_hand.is_higher_label(highest_initial_suit_label))):
                    can_win_with_suit = True

            if card_in_hand.is_same_suit(self.__trump.get_suit_id()) and (card_in_hand.has_higher_value(highest_trump_value) or (card_in_hand.has_same_value(highest_trump_value) and card_in_hand.is_higher_label(highest_trump_label))):
            ...
            if not card_in_hand.is_same_suit(self.__trump.get_suit_id()) and high_cards_played[card_in_hand.get_suit_id() - 1] == max_played_as_and_three_suit:
            ...
            if not card_in_hand.is_same_suit(self.__trump.get_suit_id()) and card_in_hand.has_higher_value(played_card.get_value()):
            ...
            if self_singed_suit is not None and not played_card.is_king() and not played_card.is_knight() and played_card.is_same_suit(self_singed_suit):
        ...
        if is_first_turn:
            ...
            # ****
            if (played_card.is_as() or played_card.is_three()) and (has_low_cards_not_trump_suit or has_cards_with_less_value):
            ...
            if played_card.is_same_suit(self.__trump.get_suit_id()) and has_lower_cards_trump_suit_to_win_round:
                # Iniciar el torn amb As o 3 no és correcte (de vegades és bo depenent si sospites que el rival no te cartes de triomf, però, en general, és una jugada horrible)
                # Restem el valor de la carta per 2
                # heuristic = -3 if played_card.is_as() or played_card.is_three() else 3
                heuristic -= played_card.get_value() - lower_card_trump_suit_to_win_round.get_value()
        ...
        # elif len(self.__round_played_cards) == self.__num_players - 1:
            ...
            if highest_trump is not None and highest_trump.is_three() and played_card.is_same_suit(self.__trump.get_suit_id()) and played_card.is_as():
                heuristic += 5
            ...
            if not wins_round and valuable_cards_played:
                ...
                if not teammate_wins_round:
                    if can_win_with_high:
                        # Podia guanyar la ronda amb un As o 3, restem 15
                        heuristic -= 15
                ...
                if can_win_with_high and (played_card.is_as() or played_card.is_three() and played_card.is_same_suit(initial_card.get_suit_id())):
                ...
                if played_card.is_as() or played_card.is_three():
                    # La guanya amb l'As o el 3 del coll de ronda
                    # Sumem 10 punts
                    heuristic += 15
                else:
            ...
            # ****
            # No es guanya la ronda, jugar cartes de valor tenint sense valor a la mà o amb cartes que podria haver guanyat la ronda és incorrecte (es resta 10)
            # if not wins_round and has_cards_with_less_value and played_card.has_higher_value(4):
            if not wins_round and not teammate_wins_round and (has_cards_with_less_value or can_win_with_high or can_win_with_suit or can_win_with_trump):
                heuristic -= 10
                # heuristic -= played_card.get_value()
            ...
            if not valuable_cards_played and has_low_cards_not_trump_suit and played_card.is_same_suit(self.__trump.get_suit_id()):
                # heuristic -= 3
                heuristic -= 30
            ...
            # Restem 5 punts
            if not can_win_with_high and not can_win_with_trump and not can_win_with_high and has_low_cards_not_trump_suit and played_card.is_same_suit(self.__trump.get_suit_id()):
                # heuristic -= 3
                heuristic -= 5
                print("heuristic 9", heuristic)

            # Està utilitzant una carta de triomf d'alt valor per guanyar la ronda tenint d'altres més baixes que li permeten guanyar la ronda i guardar-se les millors cartes
            # Restem la diferència de valor per 2
            if played_card.is_same_suit(self.__trump.get_suit_id() and (played_card.is_as() or played_card.is_three()) and has_lower_cards_trump_suit_to_win_round):
                heuristic -= (played_card.get_value() - lower_card_trump_suit_to_win_round.get_value()) * 2

                print("heuristic 9.5", heuristic)

        # S'està utilitzant un rei o cavall quan es pot utilitzar qualsevol altra carta
        # Error greu, restem 5 punts
        if not self.__is_brisca and can_use_other_than_suit_king_knight and (played_card.is_same_suit(self_singed_suit) and (played_card.is_king() or played_card.is_knight())):
            # heuristic -= 10
            heuristic -= 5
